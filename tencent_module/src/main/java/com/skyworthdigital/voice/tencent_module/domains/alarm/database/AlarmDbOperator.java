package com.skyworthdigital.voice.tencent_module.domains.alarm.database;
    ...
    public boolean insert(ContentValues values) {
        ...
        try {
            String QUIRY_ALARM_ID = "SELECT _id," + AlarmDbHelper.COL_ALARM_TIME
                    ...
                    + AlarmDbHelper.COL_ALARM_TIME + "= " + values.getAsString(AlarmDbHelper.COL_ALARM_TIME);
            Cursor cursor = dbWriter.rawQuery(QUIRY_ALARM_ID, null);

            if (cursor.moveToNext()) {
                MLog.d("alarm", "alarm already exist");
                cursor.close();
                return false;
            }
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    alarm.addAlarm(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(4));
                } while (cursor.moveToNext());
            }
            cursor.close();
        } catch (Exception e) {
