...
public class MyWakeup implements IWakeupDataListener {
    private static final String TAG = "MyWakeup";
    private static MyWakeup mInstance;
     ...
     * 获取实例
     */
    public static MyWakeup getInstance(IWakeupResultListener wkresultlistener) {
        if (mInstance == null) {
            mInstance = new MyWakeup(wkresultlistener);
        }
        return mInstance;
    }

    private MyWakeup(IWakeupResultListener wkresultlistener) {
