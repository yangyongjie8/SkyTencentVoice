...
public class MyRecognizer implements IAsrDataListener {
    ...
    private static MyRecognizer mInstance;
    private boolean mRecognizing = false;

    /**
     * 获取实例
     */
    public static MyRecognizer getInstance(IRecogListener recogListener) {
        if (mInstance == null) {
            mInstance = new MyRecognizer(recogListener);
        }
        return mInstance;
    }

    private MyRecognizer(IRecogListener recogListener) {
